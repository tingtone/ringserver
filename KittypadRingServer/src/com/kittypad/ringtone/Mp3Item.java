...
public class Mp3Item {
	...
	public Mp3Item(String UUID, String musicName, String category, Key key, String type, long size){
		this.UUID = UUID;
		...
		SearchUtils.updateFTSStuffForMp3Item(this);
	...
	public static void insert(String keyStr, String UUID, String musicName, String category, String type, long size){
		Key key = KeyFactory.createKey(Mp3Item.class.getSimpleName(), keyStr);
		Mp3Item item = new Mp3Item(UUID, musicName, category, key, type, size);
		PersistenceManager pm = PMF.get().getPersistenceManager();
		pm.makePersistent(item);
	}
	
	public static List<Mp3Item> getItems(){
		PersistenceManager pm = PMF.get().getPersistenceManager();
		Query query = pm.newQuery(Mp3Item.class);
		//	query.setOrdering("when DESC");
		List<Mp3Item> items = (List<Mp3Item>) query.execute();
		return items;
	}
